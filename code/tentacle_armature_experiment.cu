...
#ifndef TENTACLE_USE_ADSDF
#define TENTACLE_USE_ADSDF 0
#endif

#if TENTACLE_USE_ADSDF
...
#else
#define TENTACLE_OUTPUT_PREFIX "tentacle_armature"
#define TENTACLE_REPORT_LABEL "tentacle"
using ObstacleSample = geom::MeshSdfSample;
using ObstacleSampler = geom::GpuMeshSdfSampler;
#endif
...
static const std::vector<ObstacleSample> &sampleObstacleField(
	...
	return adsdf::sampleAdsdfPointsBlocking(
		...
		0.003f);
#else
	return geom::sampleMeshSdfPointsBlocking(
		samples,
		sampler,
		obstacle.mesh,
		obstacle.bvh,
...
#else
	obstacleField.mesh = geom::viewGpuMesh(&obstacleMesh_d);
	obstacleField.bvh = geom::viewGpuBvh(&obstacleBvh_d);
#endif
...
#else
	std::printf("wrote ../working/" TENTACLE_OUTPUT_PREFIX "_{obstacle,initial,optimized,guide_controls,optimized_controls}.obj and preview/control diagnostic PPMs\n");
#endif
	return 0;
}
